...
class DeepgramService extends EventEmitter {
  ...
  getStream() {
    if (this.newStreamRequired()) {
      ...
      var request =
        'encoding=mulaw&sample_rate=8000&language=' +
        this.primarylanguage +
        '&model=' +
        model +
        '&smart_format=true&filler_words=true&no_delay=true&interim_results=true&vad_turnoff=60';
      if (redact != null) {
        console.log('Redact is true');
        let redactquery = redact
          .split(',')
          .map((value) => `redact=${value}`)
          .join('&'); //creating redaction query as redact=option1&redact=option2
        request =
          'encoding=mulaw&sample_rate=8000&language=' +
          this.primarylanguage +
          '&model=' +
      ...
      try {
        this.stream = new WebSocket(
          'wss://api.deepgram.com/v1/listen?' + request,
          {
            headers: {
      ...
      this.stream.on('message', function incoming(data) {
        ...
        if (data.hasOwnProperty('channel')) {
          if (data['channel']['alternatives'][0]['transcript'] != '') {
            ...
            const timeWhenPacketStartedProcessing =
              currentTime - parseInt(durationSeconds);
...
function convert(data, offset) {
  ...
  data.channel.alternatives[0].words.forEach((element) => {
    ...
    let startEndNanos = convertNanosToString(
      Math.round((element.start - startEndSecs) * 1_000_000_000)
    );
    let endEndSecs = Math.floor(element.end);
    let endEndNanos = convertNanosToString(
      Math.round((element.end - endEndSecs) * 1_000_000_000)
    );
